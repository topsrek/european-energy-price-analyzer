...
class SmartBatchDownloader:
    ...
    def __init__(self, country_code: str = "AT", public_dir: str = "../public"):
    ...
    def find_missing_dates(self, start_date: date, end_date: date, existing_timestamps: Set[datetime]) -> Set[date]:
        ...
        # Use metadata system to find missing dates
        missing_dates = self.metadata.find_missing_dates(start_date, end_date)
        
        logger.info(f"Date range analysis ({start_date} to {end_date}):")
        logger.info(f"  Total days requested: {(end_date - start_date).days + 1}")
        logger.info(f"  Days already in metadata: {len({ts.date() for ts in existing_timestamps if start_date <= ts.date() <= end_date})}")
        ...
        for i, (start_date, end_date) in enumerate(date_ranges, 1):
            ...
            try:
                ...
                if result:
                    ...
                    logger.info(f"✓ Downloaded {len(timestamps)} records from range ({days_in_range} days)")
                else:
                    failed_ranges += 1
